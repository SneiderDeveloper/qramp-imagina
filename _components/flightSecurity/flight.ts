...
import storeFueling from '../securityForm/store'
import storeFlight from '../flight/store'
import store from './store'
import serviceListStore from '../serviceList/store/serviceList'
import { store as storeUtil, i18n, alert } from 'src/plugins/utils';
...
export default function flightController() {
  ...
  const readStatus = computed(() => {
    return !storeUtil.hasAccess('ramp.work-orders.edit-status') || disabledReadonly.value
  })
    ...
    return {
      ...
      flyFormRight: {
        ...
        statusId: {
          ...
          type: 'select',
          props: {
            vIf: storeUtil.hasAccess('ramp.work-orders.edit-status'),
            rules: [
              val => !!val || i18n.tr('isite.cms.message.fieldRequired')
            ],
        ...
        responsibleId: {
          ...
          type: "select",
          props: {
            vIf: storeUtil.hasAccess('ramp.work-orders.manage-responsible'),
            selectByDefault: true,
            readonly: disabledReadonly.value,
            label: 'Assigned to',
            clearable: true,
